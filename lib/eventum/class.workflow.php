...
class Workflow
{
     ...
     *
     * @param   integer $prj_id The project ID.
     * @param   integer $usr_id User ID who will perform the action (APP_SYSTEM_USER_ID if not changed).
     * @param   integer $issue_id The ID of the issue.
     ...
     */
    public static function handleSCMCheckins($prj_id, &$usr_id, $issue_id, $files, $username, $commit_msg)
    {
        if (!self::hasWorkflowIntegration($prj_id)) {
            return;
        }
         ...
         */
        $backend->handleSCMCheckins($prj_id, $issue_id, null, $files, $username, $commit_msg, $usr_id);
    }
