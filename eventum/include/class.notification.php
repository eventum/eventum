...
class Notification
{
    ...
    function notifyAssignedUsers($users, $issue_id)
    {
        ...
        for ($i = 0; $i < count($emails); $i++) {
            ...
            $mail = new Mail_API;
            $mail->setTextBody($text_message);
            $mail->send(Notification::getFixedFromHeader($issue_id, '', 'issue'), $emails[$i], "[#$issue_id] Assignment: " . $issue['iss_summary'], TRUE, $issue_id);
        }
    }
    ...
    function notifyNewAssignment($users, $issue_id)
    {
        ...
        for ($i = 0; $i < count($emails); $i++) {
            ...
            $mail = new Mail_API;
            $mail->setTextBody($text_message);
            $mail->send(Notification::getFixedFromHeader($issue_id, '', 'issue'), $emails[$i], "[#$issue_id] New Assignment: " . $issue['iss_summary'], TRUE, $issue_id);
        }
    }
